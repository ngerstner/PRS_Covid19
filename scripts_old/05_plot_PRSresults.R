...
setwd("/binder/mgp/datasets/2020_GWAS_Covid19/")
#sum_file <- "~/Documents/Covid19/GWAS_summaries.tsv"
sum_file <- "GWAS_summaries.tsv"
#base_path_PRS <- "/binder/mgp/datasets/2020_GWAS_Covid19/"
...
for (c in cohorts){
  for (t in traits){
    sum_tmp <- read.csv(paste0(t,"/",c,"_plink_out.txt.sscore"), 
                        header = TRUE, sep = "\t")
...
# Plot results
ggplot(result_table, aes(x=trait, y=score_avg, fill=cohort)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("PRS_Covid19.png", width = 20, height = 15)
